...
#define LN 128
#define TEMP_DIR "/tmp"
#define CONF_DIR "/acc/src/dsc/drivers/cohtdrep/lewis/coht/lrfsc/test"
...
/* ================================== */
/* Open the driver handles            */

static int module = 0;
static int modules = 0;
static int handles[LrfscDrvrMODULE_CONTEXTS] = { 0, 0 };
...
static void IErr(char *name, int *value) {
   ...
   if (value != NULL)
      printf("LrfscDrvr: [%02d] ioctl=%s arg=%d :Error\n",
	     (int) handles[module], name, (int) *value);
   else
      printf("LrfscDrvr: [%02d] ioctl=%s :Error\n",
	     (int) handles[module], name);
   perror("LrfscDrvr");
}
...
LrfscDrvrModuleAddress mad;
   ...
   if (at == Numeric) {
      ...
      if (v->Number) {
	 mod = v->Number;
	 if ((mod < 0) || (mod > modules)) mod = 0;
	 if (!LrfscOpen()) {
	    printf("Could not open /dev/lrfsc.0, No driver\n");
	    return arg;
	 }
	 if (handles[mod]) module = mod;
      }
   }

   for (mod=0; mod<modules; mod++) {
      ioctl(handles[mod],LrfscDrvrGET_MODULE_ADDRESS,&mad);
...
int debug;
   ...
   if (at == Numeric) {
      ...
      if (ioctl(handles[module],LrfscDrvrSET_SW_DEBUG,&debug) < 0) {
	 IErr("SET_SW_DEBUG",&debug);
	 return arg;
      }
   }
   debug = -1;
   if (ioctl(handles[module],LrfscDrvrGET_SW_DEBUG,&debug) < 0) {
...
int timeout;
   ...
   if (at == Numeric) {
      arg++;
      timeout = v->Number;
      if (ioctl(handles[module],LrfscDrvrSET_TIMEOUT,&timeout) < 0) {
	 IErr("SET_TIMEOUT",&timeout);
	 return arg;
      }
   }
   timeout = -1;
   if (ioctl(handles[module],LrfscDrvrGET_TIMEOUT,&timeout) < 0) {
...
int n, radix, nadr;
   ...
   while (1) {
      ...
      iob.RamFlag = ramflg;
      iob.UserArray = array;
      if (ioctl(handles[module],LrfscDrvrRAW_READ,&iob) < 0) IErr("RAW_READ",(int *) &addr);
	 ...
	 *data = strtoul(str,&cp,radix);
	 if (cp != str) {
	    if (ioctl(handles[module],LrfscDrvrRAW_WRITE,&iob) < 0) IErr("RAW_WRITE",(int *) &addr);
	 }
      }
   }
}
...
int module;
   ...
   if (ioctl(handles[module],LrfscDrvrRESET,NULL) < 0) {
      IErr("RESET",NULL);
      return arg;
   }
...
time_t tod;
   ...
   if (ioctl(handles[module],LrfscDrvrGET_VERSION,&version) < 0) {
...
LrfscDrvrInterruptBuffer ibuf;
int i, cc;
 
   arg++;

   cc = read(handles[module],&ibuf,sizeof(LrfscDrvrInterruptBuffer));
   if (cc <= 0) {
      printf("Time out or Interrupted call\n");
      return arg;
   }

   printf("Mod:%d ICnt:%d Mask:0x%X Puls:%d Cycl:%d",
	  module,
	  ibuf.ICount,
	  ibuf.Interrupt,
	  ibuf.Pulse,
   ...
   for (i=0; i<LrfscDrvrINTERRUPTS; i++) {
      if ((1 << i) & ibuf.Interrupt) printf(" %s",intr_names[i]);
   }
   printf("\n");
...
LrfscDrvrStatus stat;
   ...
   stat = 0;
   if (ioctl(handles[module],LrfscDrvrGET_STATUS,&stat) <0) {
...
int i;
   ...
   if (at == Numeric) {
	 ...
	 stat = v->Number;
	 if (ioctl(handles[module],LrfscDrvrSET_STATE,&stat) < 0) {
	    IErr("SET_STATE",NULL);
	    return arg;
	 }
   ...
   stat = 0;
   if (ioctl(handles[module],LrfscDrvrGET_STATE,&stat) < 0) {
      IErr("GET_STATE", NULL);
      return arg;
   }
...
int i, diag, sval;
   ...
   if (ioctl(handles[module],LrfscDrvrGET_DIAG_CHOICE,&dchoice) < 0) {
   ...
   if (sval != -1) {
      ...
      if (ioctl(handles[module],LrfscDrvrSET_DIAG_CHOICE,&dchoice) < 0) {
...
LrfscDrvrResCtrl resc;
   ...
   if (at == Numeric) {
	 ...
	 if (ioctl(handles[module],LrfscDrvrGET_RES_CTRL, &resc) < 0) {
	 ...
	 if (ioctl(handles[module],LrfscDrvrSET_RES_CTRL, &resc) < 0) {
...
int i, msk;
   ...
   if (at == Numeric) {
      ...
      if (v->Number <= 0x0F) {
	 anls = v->Number;
	 if (ioctl(handles[module],LrfscDrvrSET_ANALOGUE_SWITCH,&anls) < 0) {
   ...
   anls = 0;
   if (ioctl(handles[module],LrfscDrvrGET_ANALOGUE_SWITCH,&anls) < 0) {
...
LrfscDrvrSoftSwitch sfts;
   ...
   if (at == Numeric) {
	 ...
	 sfts = v->Number;
	 if (ioctl(handles[module],LrfscDrvrSET_SOFT_SWITCH,&sfts) < 0) {
   ...
   sfts = 0;
   if (ioctl(handles[module],LrfscDrvrGET_SOFT_SWITCH,&sfts) < 0) {
...
int i;
   ...
   if (at == Numeric) {
	 ...
	 if (ioctl(handles[module],LrfscDrvrGET_COEFFICIENTS,&cobuf) < 0) {
	 ...
	 if (at == Numeric) {
	    ...
	    if (ioctl(handles[module],LrfscDrvrSET_COEFFICIENTS,&cobuf) < 0) {
...
LrfscDrvrDiagSnapShot snap;
   ...
   if (at == Numeric) {
      ...
      if (ioctl(handles[module],LrfscDrvrSET_SNAP_SHOT_TIME,&snap) < 0) {
	 IErr("SET_SNAP_SHOT_TIME",(int *) &(v->Number));
	 return arg;
      }
   }

   if (ioctl(handles[module],LrfscDrvrGET_SNAP_SHOTS,&snap) < 0) {
...
LrfscDrvrPicSetBuf pic;
   ...
   if (ioctl(handles[module],LrfscDrvrGET_PIC,&pic) < 0) {
      IErr("GET_PIC",NULL);
      return arg;
   }
   ...
   if (at == Numeric) {
	 ...
	 pic.KI = (signed short) v->Number;
      ...
      if (ioctl(handles[module],LrfscDrvrSET_PIC,&pic) < 0) {
	 IErr("SET_PIC",NULL);
	 return arg;
      }
   }
...
unsigned long pl, plmx;
   ...
   if (at == Numeric) {
      arg++;
      plmx =  v->Number;
      if (ioctl(handles[module],LrfscDrvrSET_MAX_PULSE_LENGTH,&plmx) < 0) {
	 IErr("SET_MAX_PULSE_LENGTH",(int *) &plmx);
	 return arg;
      }
   }

   if (ioctl(handles[module],LrfscDrvrGET_MAX_PULSE_LENGTH,&plmx) < 0) {
      IErr("GET_MAX_PULSE_LENGTH",(int *) &plmx);
      return arg;
   }

   if (ioctl(handles[module],LrfscDrvrGET_PULSE_LENGTH,&plmx) < 0) {
...
unsigned long pcy, ncy;
   ...
   if (at == Numeric) {
	 ...
	 cyc = ncy = v->Number;
	 if (ioctl(handles[module],LrfscDrvrSET_NEXT_CYCLE,&ncy) < 0) {
   ...
   if (ioctl(handles[module],LrfscDrvrGET_PRES_CYCLE,&pcy) < 0) {
...
int i;
   ...
   if (at == Numeric) {
	 ...
	 if (ioctl(handles[module],LrfscDrvrGET_CYCLE_CONFIG,&cbuf) < 0) {
...
int i;
   ...
   if (YesNo(fnam, "Write Hardware")) {
      if (ioctl(handles[module],LrfscDrvrSET_CYCLE_CONFIG,&cbuf) < 0) {
...
unsigned long plen;
   ...
   if (ioctl(handles[module],LrfscDrvrSET_SKIP_START,&SampStart) < 0) {
      IErr("LrfscDrvrSET_SKIP_START",(int *) &SampStart);
      return arg;
   }

   if (SampSkip == 0) {
      if (ioctl(handles[module],LrfscDrvrGET_PULSE_LENGTH,&plen) < 0) {
   ...
   if (ioctl(handles[module],LrfscDrvrSET_SKIP_COUNT,&SampSkip) < 0) {
      IErr("LrfscDrvrSET_SKIP_COUNT",(int *) &SampSkip);
      return arg;
   }

   if (ioctl(handles[module],LrfscDrvrSET_PULSE,&SampPulse) < 0) {
...
unsigned int tm;
   ...
   simulate = 0;
   if (ioctl(handles[module],LrfscDrvrGET_DIAGNOSTICS,&dbuf) < 0) {
